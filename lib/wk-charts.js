...
angular.module('wk.chart').factory('dataManagerFactory', function($log) {
  ...
  mergeSeriesKeys = function(aOld, aNew) {
  ...
  merge = function() {
    var getEndLayerKeyMap, getMergedEnd, getMergedStart, getStartLayerKeyMap, me, _dataNew, _dataOld, _isOrdinal, _keyNew, _keyOld, _keyScale, _layerKeysNew, _layerKeysOld, _mergedKeys, _mergedLayerKeys, _valueScale;
    _dataOld = [];
    _dataNew = [];
    _keyOld = [];
    _keyNew = [];
    ...
    me.data = function(data) {
      var _endLayerMap, _startLayerMap;
      _dataOld = _dataNew;
      _keyOld = _keyNew;
    ...
    getMergedStart = (function(_this) {
      return function() {
        var borderKey, borderKeyLeft, cur, i, lastKey, lastOld, ret;
        ret = [];
        ...
        while (i < _mergedKeys.length) {
          ...
          if (cur.iOld !== void 0) {
            ...
            ret.push({
              added: true,
              key: (cur.atBorder ? borderKey(cur) : lastKey),
    ...
    getStartLayerKeyMap = function(mergedKeys) {
      var first, l, last, map, _i, _len;
      map = {};
      first = _layerKeysOld[0];
      last = void 0;
      for (_i = 0, _len = mergedKeys.length; _i < _len; _i++) {
        l = mergedKeys[_i];
        if (l.iOld !== void 0) {
          map[l.key] = l.key;
          last = l.key;
        }
        map[l.key] = last || first;
      }
      $log.info('startMap', map);
      return map;
    };
    me.animationStartLayers = function() {
      var layerKeys, layerMap, series;
      layerKeys = _mergedLayerKeys.map(function(d) {
        return d.key;
      });
    ...
    getMergedEnd = (function(_this) {
      return function() {
        var borderKey, borderKeyRight, cur, i, lastKey, lastNew, ret;
        ret = [];
    ...
    getEndLayerKeyMap = function(mergedKeys) {
      var i, l, last, lastSeen, map;
      map = {};
...
angular.module('wk.chart').directive('area', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var area, brush, drawPath, host, layoutData, markers, offset, setAnimationEnd, setAnimationStart, ttHelper, xData, _id, _initialOpacity, _scaleList, _showMarkers, _tooltip;
      host = controller.me;
      _tooltip = void 0;
      _scaleList = {};
      _showMarkers = false;
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        ...
        area = d3.svg.area().x(function(d) {
          ...
          return y.scale()(d.added || d.deleted ? 0 : d.value);
        }).y1(function(d) {
          return y.scale()(0);
        });
        ...
        markers.x(function(d) {
          return x.scale()(d.key) + (x.isOrdinal() ? x.scale().rangeBand() / 2 : 0);
        }).y(function(d) {
          return y.scale()(d.added || d.deleted ? 0 : d.value);
        }).color(function(d) {
          return color.scale()(d.layerKey);
      ...
      brush = function(axis, idxRange) {
        ...
        if (axis.isOrdinal()) {
          lines.attr('d', function(d) {
            return area(d.value.slice(idxRange[0], idxRange[1] + 1));
          });
        } else {
          lines.attr('d', function(d) {
            return area(d.values);
...
angular.module('wk.chart').directive('areaStacked', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var area, brush, drawPath, host, layers, layoutData, markers, offs, offset, scaleY, setAnimationEnd, setAnimationStart, stack, stackLayout, ttHelper, xData, _id, _scaleList, _showMarkers, _tooltip;
      host = controller.me;
...
angular.module('wk.chart').directive('areaStackedVertical', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var area, brush, drawPath, host, layers, layoutData, markers, offs, offset, setAnimationEnd, setAnimationStart, stack, stackLayout, ttHelper, xData, _id, _scaleList, _showMarkers, _tooltip;
      host = controller.me;
      stack = d3.layout.stack();
      offset = 'zero';
      layers = null;
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        ...
        markers.x(function(d) {
          ...
          return y.scale()(d.key) + (y.isOrdinal() ? y.scale().rangeBand() / 2 : 0);
        }).color(function(d) {
          return color.scale()(d.layerKey);
        });
        return layers.call(markers, doAnimate);
...
angular.module('wk.chart').directive('areaVertical', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var area, brush, drawPath, host, layoutData, markers, offset, setAnimationEnd, setAnimationStart, ttHelper, xData, _circles, _id, _initialOpacity, _scaleList, _showMarkers, _tooltip, _ttHighlight;
      host = controller.me;
      _tooltip = void 0;
      _ttHighlight = void 0;
      _circles = void 0;
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        ...
        area = d3.svg.area().x(function(d) {
          ...
          return x.scale()(d.added || d.deleted ? x.scale()(0) : d.value);
        }).y1(function(d) {
          return x.scale()(0);
        });
        layers = this.selectAll(".wk-chart-layer").data(data, function(d) {
        ...
        markers.isVertical(true).x(function(d) {
          return x.scale()(d.added || d.deleted ? 0 : d.value);
        }).y(function(d) {
          return y.scale()(d.key) + (y.isOrdinal() ? y.scale().rangeBand() / 2 : 0);
        }).color(function(d) {
          return color.scale()(d.layerKey);
...
/**
  ...
  @usesDimension color [type=category20]
 */
angular.module('wk.chart').directive('columnStacked', function($log, utils, barConfig, dataManagerFactoryV2) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var barOuterPaddingOld, barPaddingOld, config, draw, drawBrush, host, initial, layers, stack, ttEnter, v2Mgr, _id, _merge, _mergeLayers, _scaleList, _selected, _tooltip;
      host = controller.me;
...
angular.module('wk.chart').directive('line', function($log, behavior, utils, dataManagerFactory, tooltipHelperFactory, markerFactory) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var brush, drawPath, host, layoutData, line, markers, offset, setAnimationEnd, setAnimationStart, ttHelper, xData, _id, _scaleList, _showMarkers, _tooltip;
      host = controller.me;
      _tooltip = void 0;
      _showMarkers = false;
      _scaleList = {};
...
angular.module('wk.chart').directive('lineVertical', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  return {
    ...
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var brush, brushStartIdx, drawPath, host, layoutData, line, markers, offset, setAnimationEnd, setAnimationStart, ttHelper, xData, _id, _scaleList, _showMarkers, _tooltip;
      host = controller.me;
      brushStartIdx = 0;
      _tooltip = void 0;
      _showMarkers = false;
...
angular.module('wk.chart').directive('pie', function($log, utils) {
  ...
  return {
    ...
    require: '^layout',
    link: function(scope, element, attrs, controller) {
      var animationDuration, draw, highlightSelected, initialShow, inner, labels, layout, outer, pieBox, polyline, selectionHandler, selectionOffset, ttEnter, _id, _merge, _scaleList, _selected, _showLabels, _tooltip;
      layout = controller.me;
      _id = "pie" + (pieCntr++);
      inner = void 0;
      outer = void 0;
      ...
      initialShow = true;
      draw = function(data, options, x, y, color, size) {
        var arcTween, highlArc, innerArc, key, midAngle, outerArc, pie, r, segments;
        ...
        innerArc = d3.svg.arc().outerRadius(r * (_showLabels ? 0.8 : 0.9)).innerRadius(0);
        highlArc = d3.svg.arc().outerRadius(r * (_showLabels ? 0.85 : 1)).innerRadius(r * (_showLabels ? 0.8 : 0.95));
...
/**
  ...
  @example
 */
angular.module('wk.chart').directive('rangeArea', function($log, utils, tooltipUtils) {
  var lineCntr;
  lineCntr = 0;
  return {
    restrict: 'A',
    require: 'layout',
    link: function(scope, element, attrs, controller) {
      var area, areaBrush, brush, draw, host, layerKeys, m, offset, ttEnter, ttMoveData, ttMoveMarker, _circles, _dataOld, _id, _layout, _pathArray, _pathValuesNew, _pathValuesOld, _scaleList, _showMarkers, _tooltip, _ttHighlight;
      host = controller.me;
      layerKeys = [];
      _layout = [];
      _dataOld = [];
      ...
      ttMoveData = function(idx) {
        ...
        ttLayers = [];
        ttLayers.push({
          name: _scaleList.rangeY.lowerProperty(),
          value: _scaleList.rangeY.formatValue(_scaleList.rangeY.lowerValue(datum)),
          color: {
            'background-color': _pathArray[idx].color
          }
        });
        ttLayers.push({
          name: _scaleList.rangeY.upperProperty(),
          value: _scaleList.rangeY.formatValue(_scaleList.rangeY.upperValue(datum)),
          color: {
            'background-color': _pathArray[idx].color
          }
        });
        this.headerName = _scaleList.x.axisLabel();
        this.headerValue = _scaleList.x.formattedValue(datum);
        return this.layers = this.layers.concat(ttLayers);
      };
      ttMoveMarker = function(idx) {
        ...
        _circles = _circles.data(m, function(d) {
          return d.i;
        });
        ...
        return this.attr('transform', "translate(" + (_scaleList.x.scale()(_pathArray[idx].xv) + offset) + ")");
      };
      draw = function(data, options, x, y, color, size, shape, rangeX, rangeY) {
        var areaNew, areaOld, enter, i, layer, mergedX, newFirst, oldFirst, path, v, val, _i, _len;
        mergedX = utils.mergeSeriesUnsorted(x.value(_dataOld), x.value(data));
        _layout = [];
        _pathValuesNew = [];
        _pathValuesNew = data.map(function(d) {
          return {
            x: x.map(d),
            y1: rangeY.scale()(rangeY.upperValue(d)),
            y0: rangeY.scale()(rangeY.lowerValue(d)),
            xv: x.value(d),
            yv1: rangeY.upperValue(d),
            yv0: rangeY.lowerValue(d),
            color: color.scale()(0),
            data: d
          };
        });
        ...
        for (i = _i = 0, _len = mergedX.length; _i < _len; i = ++_i) {
          val = mergedX[i];
          v = {
            x: val[2]
          };
        ...
        enter = path.enter().append('path').attr('class', 'wk-chart-area-path').style('stroke', function(d) {
          return d.color;
        }).style('fill', function(d) {
          return d.color;
        }).style('opacity', 0).style('pointer-events', 'none');
        path.attr('transform', "translate(" + offset + ")").attr('d', function(d) {
        ...
        path.exit().transition().duration(options.duration).style('opacity', 0).remove();
        _dataOld = data;
      ...
      host.lifeCycle().on('configure', function() {
        _scaleList = this.getScales(['x', 'rangeY', 'color']);
        this.layerScale('color');
        this.getKind('rangeY').domainCalc('extent').resetOnNewData(true);
        this.getKind('x').resetOnNewData(true).domainCalc('extent');
        _tooltip = host.behavior().tooltip;
        _tooltip.markerScale(_scaleList.x);
        _tooltip.on("enter." + _id, ttEnter);
...
/**
  ...
  @usesDimension color [type=category20]
 */
angular.module('wk.chart').directive('rangeBars', function($log, utils, barConfig, wkChartMargins) {
  var sBarCntr;
  sBarCntr = 0;
  return {
    restrict: 'A',
    ...
    link: function(scope, element, attrs, controller) {
      ...
      ttEnter = function(data) {
        this.headerName = _scaleList.y.axisLabel();
        this.headerValue = _scaleList.y.formattedValue(data.data);
        this.layers.push({
          name: _scaleList.rangeX.lowerProperty(),
          value: _scaleList.rangeX.formatValue(_scaleList.rangeX.lowerValue(data.data)),
          color: {
            'background-color': _scaleList.color.map(data.data)
          }
        });
        return this.layers.push({
          name: _scaleList.rangeX.upperProperty(),
          value: _scaleList.rangeX.formatValue(_scaleList.rangeX.upperValue(data.data)),
          color: {
            'background-color': _scaleList.color.map(data.data)
          }
        });
      ...
      draw = function(data, options, x, y, color, size, shape, rangeX, rangeY) {
        ...
        bars.select('text').text(function(d) {
          return rangeX.formatValue(rangeX.lowerValue(d.data));
        }).transition().duration(options.duration).attr('y', function(d) {
          return d.height / 2;
        }).attr('x', function(d) {
          return d.x + wkChartMargins.dataLabelPadding.hor;
      ...
      host.lifeCycle().on('configure', function() {
        _scaleList = this.getScales(['rangeX', 'y', 'color']);
        this.getKind('rangeX').domainCalc('extent').resetOnNewData(true);
        this.getKind('y').resetOnNewData(true).rangePadding(config).scaleType('ordinal');
        _tooltip = host.behavior().tooltip;
        _selected = host.behavior().selected;
        return _tooltip.on("enter." + _id, ttEnter);
...
/**
  ...
  @usesDimension color [type=category20]
 */
angular.module('wk.chart').directive('rangeColumn', function($log, utils, barConfig, wkChartMargins) {
  var sBarCntr;
  sBarCntr = 0;
  return {
    restrict: 'A',
    ...
    link: function(scope, element, attrs, controller) {
      ...
      ttEnter = function(data) {
        this.headerName = _scaleList.x.axisLabel();
        this.headerValue = _scaleList.x.formattedValue(data.data);
        this.layers.push({
          name: _scaleList.rangeY.upperProperty(),
          value: _scaleList.rangeY.formatValue(_scaleList.rangeY.upperValue(data.data)),
          color: {
            'background-color': _scaleList.color.map(data.data)
          }
        });
        return this.layers.push({
          name: _scaleList.rangeY.lowerProperty(),
          value: _scaleList.rangeY.formatValue(_scaleList.rangeY.lowerValue(data.data)),
          color: {
            'background-color': _scaleList.color.map(data.data)
          }
        });
      ...
      draw = function(data, options, x, y, color, size, shape, rangeX, rangeY) {
        ...
        enter = columns.enter().append('g').attr('class', 'wk-chart-column').attr('transform', function(d, i) {
          return "translate(" + (initial ? d.x : _merge.addedPred(d).x + _merge.addedPred(d).width + (i ? barPaddingOld / 2 : barOuterPaddingOld)) + "," + (d.y - d.height) + ") scale(" + (initial ? 1 : 0) + ",1)";
        });
        enter.append('rect').attr('class', 'wk-chart-rect wk-chart-selectable').attr('height', function(d) {
          return d.height;
        }).attr('width', function(d) {
        ...
        columns.transition().duration(options.duration).attr("transform", function(d) {
          return "translate(" + d.x + ", " + (d.y - d.height) + ") scale(1,1)";
        });
        columns.select('rect').transition().duration(options.duration).attr('width', function(d) {
          return d.width;
        }).attr('height', function(d) {
        ...
        columns.select('text').text(function(d) {
          return rangeY.formatValue(rangeY.upperValue(d));
        }).transition().duration(options.duration).attr('x', function(d) {
          return d.width / 2;
        }).style('opacity', host.showDataLabels() ? 1 : 0);
        columns.exit().transition().duration(options.duration).attr('transform', function(d) {
...
angular.module('wk.chart').factory('behaviorTooltip', function($log, $document, $rootScope, $compile, $templateCache, wkChartTemplates) {
  ...
  behaviorTooltip = function() {
    ...
    tooltipEnter = function() {
      ...
      return _tooltipDispatch.enter.apply(_templScope, [value]);
    };
...
angular.module('wk.chart').factory('scale', function($log, legend, formatDefaults, wkChartScales, wkChartLocale) {
  var scale;
  scale = function() {
    var calcDomain, keys, layerMax, layerMin, layerTotal, me, parsedValue, _axis, _axisLabel, _axisOrient, _axisOrientOld, _calculatedDomain, _chart, _domain, _domainCalc, _exponent, _id, _inputFormatFn, _inputFormatString, _isHorizontal, _isOrdinal, _isVertical, _kind, _layerExclude, _layerProp, _layout, _legend, _lowerProperty, _outputFormatFn, _outputFormatString, _parent, _property, _range, _rangeOuterPadding, _rangePadding, _resetOnNewData, _reverse, _rotateTickLabels, _scale, _scaleType, _showAxis, _showGrid, _showLabel, _tickFormat, _tickValues, _ticks, _upperProperty;
    _id = '';
...
angular.module('wk.chart').factory('markerFactory', function($log, d3Animation) {
  ...
  return markers = function() {
    var me, _active, _color, _duration, _initialOpacity, _isVertical, _markerSelection, _opacity, _x, _y;
    _x = void 0;
    _y = void 0;
    _color = void 0;
    _active = false;
    ...
    me = function(s, doAnimate) {
      ...
      _markerSelection = s;
      if (_active) {
        m = s.selectAll('.wk-chart-marker').data(function(d) {
          return d.values;
        }, function(d, i) {
          return i;
        });
        m.enter().append('circle').attr('class', 'wk-chart-marker').style('fill', _color).attr('r', 5).style('pointer-events', 'none').style('opacity', _initialOpacity);
        mUpdate = doAnimate ? m.transition().duration(_duration) : m;
        mUpdate.attr('cx', _x).attr('cy', _y).style('opacity', function(d) {
          return (d.added || d.deleted ? 0 : 1) * _opacity;
        });
        mExit = doAnimate ? m.exit().transition().duration(_duration) : m.exit();
        return mExit.remove();
      } else {
        return s.selectAll('.wk-chart-marker').transition().duration(_duration).style('opacity', 0).remove();
      }
    };
    me.brush = function(selection, idxRange) {
      ...
      if (idxRange) {
        return _markerSelection.selectAll('.wk-chart-marker').each(function(d, i) {
        ...
        return _markerSelection.selectAll('.wk-chart-marker').attr(c, v);
      }
    };
...
angular.module('wk.chart').factory('tooltipHelperFactory', function($log) {
  ...
  helperCnt = 0;
  ttHelpers = function() {
    var me, _colorScale, _getLayoutIdx, _id, _keyScale, _layout, _value, _valueScale;
    _keyScale = void 0;
    _valueScale = void 0;
    _colorScale = void 0;
    _layout = void 0;
    ...
    _getLayoutIdx = function(idx) {
      var d, domainVal, i, key, _i, _len, _ref;
      if (_keyScale.isOrdinal()) {
        domainVal = _keyScale.scale().domain()[idx];
        if (idx === domainVal) {
          return idx;
        }
    ...
    me.moveData = function(idx) {
      var lIdx, ttLayers;
      lIdx = _getLayoutIdx(idx);
      ttLayers = _layout.filter(function(d) {
        return !d.deleted;
      }).map(function(d) {
        return {
          name: d.layerKey,
          value: _valueScale.formatValue(d.values[lIdx].value),
          color: {
            'background-color': _colorScale.scale()(d.layerKey)
          }
        };
      });
      this.headerName = _keyScale.axisLabel();
      this.headerValue = _keyScale.formattedValue(_layout[0].values[lIdx].data.data);
      return this.layers = this.layers.concat(ttLayers);
    };
    me.moveMarkers = function(idx) {
      var enter, lIdx, offset, _circles;
      lIdx = _getLayoutIdx(idx);
      _circles = this.selectAll(".wk-chart-tt-marker-" + _id).data(_layout.filter(function(d) {
        return !d.deleted;
      }), function(d) {
        return d.layerKey;
      });
      ...
      enter.append('circle').attr('class', 'wk-chart-tt-marker').attr('r', 9).style('fill', function(d) {
        return _colorScale.scale()(d.layerKey);
      }).style('fill-opacity', 0.3).style('stroke', function(d) {
        return _colorScale.scale()(d.layerKey);
      }).style('pointer-events', 'none');
      enter.append('circle').attr('class', 'wk-chart-tt-marker').attr('r', 4).style('fill', function(d) {
        return _colorScale.scale()(d.layerKey);
      }).style('fill-opacity', 0.6).style('stroke', 'white').style('pointer-events', 'none');
      _circles.selectAll('circle').attr((_keyScale.isHorizontal() ? 'cy' : 'cx'), function(d) {
        return _valueScale.scale()(_value(d.values[lIdx]));
      });
      _circles.exit().remove();
      offset = _keyScale.isOrdinal() ? _keyScale.scale().rangeBand() / 2 : 0;
      if (_keyScale.isHorizontal()) {
        return this.attr('transform', "translate(" + (_keyScale.map(_layout[0].values[lIdx].data.data) + offset) + ")");
      } else {
        return this.attr('transform', "translate(0," + (_keyScale.map(_layout[0].values[lIdx].data.data) + offset) + ")");
      }
    };
    return me;
  };
...
angular.module('wk.chart').service('modelTypes', function($log, wkChartScales) {
  var areaStacked, areaStackedVertical, assign, axis, base, brush, brushed, dim, geojson, getScaleTypes, labels, layout, legend, markers, outerPadding, padding, projection, property, propertyType, range, selection, tooltips, valuesLegend;
