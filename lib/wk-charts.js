...
angular.module('wk.chart').factory('dataLabelFactory', function($log, wkChartMargins) {
  ...
  return dataLabels = function() {
    var _active, _anchor, _duration, _keyAxis, _keyScale, _margin, _style, _valueAxis, _valueScale, me;
    _active = false;
    _keyScale = void 0;
    _valueScale = void 0;
    _keyAxis = void 0;
    ...
    me = function(s, doAnimate, style, backgroundStyle) {
      ...
      if (textGroup.empty()) {
        textGroup = s.append('g').attr('class', 'wk-chart-data-label').style('opacity', 0).attr('transform', function(d) {
          ...
          v2 = _margin + (_keyScale.isHorizontal() ? Math.min(_valueScale.scale()(0), _valueScale.scale()(d.targetValue)) : Math.max(_valueScale.scale()(0), _valueScale.scale()(d.targetValue)));
          return 'translate(' + (_keyScale.isHorizontal() ? v1 + ',' + v2 : v2 + ',' + v1) + ')';
        });
    ...
    me.brush = function(selection, idxRange) {
      ...
      if (_active) {
        ...
        if (idxRange) {
          ...
          return _markerSelection.selectAll(".wk-chart-marker-" + _id).attr(c, vOrd).style('opacity', function(d, i) {
            if (idxRange[0] <= i && i <= idxRange[1]) {
              return 1;
            } else {
              return 0;
 ...
 */
angular.module('wk.chart').directive('brush', function($log, behavior) {
  var brushId;
  brushId = 0;
  return {
    restrict: 'A',
    ...
    link: function(scope, element, attrs, controllers) {
      var _brushAreaSelection, _brushGroup, _id, _isAreaBrush, _selectables, brush, chart, host, layout, ref, ref1, ref2, x, xScale, y, yScale;
      ...
      host = chart || layout;
      brush.x(x).y(y);
      scope.$watch('brushExtent', function(newVal) {
        if (_.isArray(newVal) && newVal.length === 0) {
          return brush.clearBrush();
        } else {
          return brush.setExtent(newVal);
        }
      });
...
angular.module('wk.chart').factory('behaviorBrush', function($log, $window, d3Animation) {
  var behaviorBrush;
  behaviorBrush = function() {
    var _area, _axis, _axisG, _brushAttr, _brushAxis, _brushElements, _brushEvents, _brushVisible, _chart, _chartAreaBg, _container, _cursor, _data, _empty, _fixAttr, _fixAxis, _horizontal, _idxRange, _labelInAxis, _lastPos, _pos1, _pos1Start, _pos1Value, _pos2, _pos2Start, _pos2Value, _scaleHeight, _scaleWidth, _selectedDomain, _selectedKeys, _startPos, _svg, _tooltip, _x, _y, brushDispatch, brushEnd, brushMove, brushStart, clearBrush, clearSelection, extentMove, extentStart, getAxisValue, handle1Move, handle1Start, handle2Move, handle2Start, limit, me, mousePos, positionBrush, resizeExtent, setPos1, setPos2, setSelection, setText;
    _labelInAxis = false;
    _area = void 0;
    _container = void 0;
    _chart = void 0;
    ...
    _axisG = void 0;
    _chartAreaBg = void 0;
    _brushElements = void 0;
    _brushVisible = void 0;
    _scaleWidth = 0;
    _scaleHeight = 0;
    _idxRange = [];
    _selectedKeys = [];
    ...
    me.container = function(val, axisSizing, scaleWidth, scaleHeight) {
      if (arguments.length === 0) {
        return _container;
      }
      _container = val;
    ...
    me.setExtent = function(val) {
      ...
      if (_.isArray(val) && val.length === 2) {
        ...
        _brushElements.style({
          visibility: null,
          'pointer-events': 'all'
        });
        return positionBrush();
      }
    };
    getAxisValue = function(pos) {
      return _axis.value(_axis.find(_axis.invert(pos)));
    };
    mousePos = function() {
      var p;
      p = _horizontal ? d3.mouse(_area.node())[0] : d3.mouse(_area.node())[1];
      if (p < 0) {
        return 0;
      } else if (p > limit()) {
        return limit(p);
    ...
    setPos1 = function(pos) {
      _pos1 = pos;
      return _pos1Value = getAxisValue(_pos1);
    };
    setPos2 = function(pos) {
      _pos2 = pos;
      return _pos2Value = getAxisValue(_pos2);
    };
    setSelection = function() {
      var band, i, keys, l, lowerIdx, maxVal, minVal, pos1Idx, pos1Value, pos2Idx, pos2Value, range, reverse, upperIdx;
      if (_axis.isOrdinal()) {
        range = _axis.range();
        ...
        if (upperIdx >= lowerIdx || (_axis.isVertical() && upperIdx <= lowerIdx)) {
        ...
        return _container.selectAll('.wk-chart-selectable').each(function(d) {
          var ref, ref1;
          return d3.select(this).classed('wk-chart-selected', (ref1 = d.key, indexOf.call(_selectedKeys, ref1) >= 0)).classed('wk-chart-not-selected', !(ref = d.key, indexOf.call(_selectedKeys, ref) >= 0));
        });
      } else {
        ...
        return _brushEvents.brush(_idxRange, _selectedKeys, _selectedDomain);
      }
    };
    ...
    resizeExtent = function(noAnimation) {
      var hv, hvp, lv, lvp, newKeys;
      if (_axis) {
        if (_axis.isOrdinal()) {
          ...
          if (newKeys.length > 0) {
            ...
            if (lvp < hvp && _pos1 < _pos2 || lvp > hvp && _pos1 > _pos2) {
              _pos1 = lvp;
              _pos1Value = lv;
              _pos2 = hvp;
              _pos2Value = hv;
            ...
            return positionBrush(!noAnimation);
          } else {
            return clearBrush();
          }
        } else {
          if (_pos1Value && _pos2Value) {
    ...
    clearBrush = function() {
      ...
      _svg.selectAll('.wk-chart-brush-extent').attr(_brushAxis, 0).attr(_brushAttr, _horizontal ? _scaleWidth : _scaleHeight);
      _svg.selectAll('.wk-chart-brush-rect1').attr(_brushAttr, 0);
      _svg.selectAll('.wk-chart-brush-rect2').attr(_brushAxis, _horizontal ? _scaleWidth : _scaleHeight).attr(_brushAttr, 0);
      d3.select('body').style('cursor', void 0);
      clearSelection();
      brushEnd();
      return _empty = true;
    };
    ...
    positionBrush = function(animate) {
      var _attrLimit, _maxPos, _minPos, empty, l1Pos, l2Pos, lPos, sel, t1Size, t2Size;
      if ((_pos1 === 0 && _pos2 === limit()) || (_pos1 === 0 && _pos2 === 0) || (_pos1 === limit() && _pos2 === limit())) {
        return clearBrush();
      } else {
        sel = function(s) {
          if (typeof s === 'string') {
          ...
          if (animate) {
            return s.transition().duration(d3Animation.duration);
          } else {
            return s;
          }
        };
        empty = false;
        _attrLimit = _horizontal ? _scaleWidth : _scaleHeight;
        ...
        if (_horizontal) {
          lPos = _labelInAxis ? _scaleHeight + 4 : 0;
          ...
          l1Pos = _labelInAxis ? -t1Size.width : _scaleWidth - t1Size.width;
          l2Pos = _labelInAxis ? -t2Size.width : _scaleWidth - t2Size.width;
    ...
    brushDispatch = function() {
      var p;
      p = mousePos();
      if (p > _pos1 - 4 && p <= _pos1) {
        handle1Start();
      } else if (p > _pos1 && p < _pos2) {
        extentStart();
      } else if (p >= _pos2 && p < _pos2 + 4) {
        handle2Start();
      } else {
        brushStart();
      }
      return _brushEvents.brushStart();
    };
    ...
    handle1Start = function(d) {
      ...
      w.on('mouseup.brush', brushEnd);
      return d3.select('body').style('cursor', _cursor);
    };
    handle2Start = function(d) {
      var w;
      d3.event.stopPropagation();
      _tooltip.hide(true);
      w = d3.select($window);
    ...
    extentMove = function(d) {
      var delta, p1, p2;
      d3.event.stopPropagation();
      delta = mousePos() - _startPos;
      if (_pos1Start + delta >= 0 && _pos2Start + delta <= limit()) {
        p1 = _pos1Start + delta;
        p2 = _pos2Start + delta;
      } else if (delta <= 0) {
        p1 = 0;
    ...
    brushEnd = function(d) {
      ...
      if (_tooltip) {
        _.defer(function() {
          return _tooltip.hide(false);
        });
      }
      w = d3.select($window);
      w.on('mousemove.brush', void 0);
      w.on('mouseup.brush', void 0);
      d3.select('body').style('cursor', void 0);
      return _brushEvents.brushEnd(_idxRange, _selectedKeys, _selectedDomain);
    };
    return me;
  };
...
angular.module('wk.chart').factory('behavior', function($log, $window, behaviorTooltip, behaviorSelect, behaviorBrush) {
  var behavior;
  behavior = function() {
    var _brush, _selection, _tooltip, chart, chartArea, container, svg;
    ...
    container = function(container, axisSizing, width, height) {
...
angular.module('wk.chart').factory('behaviorTooltip', function($log, $document, $rootScope, $compile, $templateCache, wkChartTemplates) {
  ...
  behaviorTooltip = function() {
    ...
    forwardToBrush = function(e) {
      var brush_elm, eventCopy;
      brush_elm = _container.select('.wk-chart-background').node();
      eventCopy = document.createEvent("MouseEvents");
      eventCopy.initMouseEvent('mousedown', true, true, d3.event.view, d3.event.detail, d3.event.pageX || d3.event.layerX, d3.event.pageY || d3.event.layerY, d3.event.clientX, d3.event.clientY, d3.event.ctrlKey, d3.event.altKey, d3.event.shiftKey, d3.event.metaKey, d3.event.button, d3.event.relatedTarget);
      return brush_elm.dispatchEvent(eventCopy);
    };
...
angular.module('wk.chart').factory('container', function($log, $window, wkChartMargins, wkChartTemplates, $templateCache) {
  ...
  containerCnt = 0;
  container = function() {
    var _behavior, _chart, _chartArea, _chartAreaDiv, _container, _containerId, _data, _duration, _element, _elementSelection, _genBrushLabel, _genChartFrame, _gridArea, _innerHeight, _innerWidth, _layouts, _legends, _margin, _overlay, _removeAxis, _removeLabel, _spacedContainer, _svg, _titleHeight, dataLabelHeight, dataLabelWidth, drawAndPositionText, drawAxis, drawGrid, drawPadding, drawTitleArea, getAxisRect, me, measureText, tickLabelsAnchor, tickLabelsShift;
    me = function() {};
    ...
    me.scaleWidth = function() {
      return _innerWidth - drawPadding;
    };
    me.scaleHeight = function() {
      return _innerHeight - drawPadding;
    };
    me.margins = function() {
      return _margin;
    };
    ...
    drawAxis = function(dim) {
      ...
      if (dim.rotateTickLabels()) {
        axis.selectAll(".wk-chart-" + (dim.axisOrient()) + ".wk-chart-axis .tick>text").attr({
          dy: '0.35em'
        }).attr('transform', function(d) {
          return tickLabelsShift(dim);
        }).style('text-anchor', tickLabelsAnchor(dim)).style('pointer-events', 'none').style(dim.tickLabelStyle());
      } else {
        axis.selectAll(".wk-chart-" + (dim.axisOrient()) + ".wk-chart-axis .tick>text").attr('transform', null).style('pointer-events', 'none').style(dim.tickLabelStyle());
      }
    ...
    _genBrushLabel = function() {
      this.append('rect');
      return this.append('text').attr('class', 'wk-chart-brush-label-text').attr('text-anchor', 'left');
    };
    _genChartFrame = function() {
      ...
      mask.append('rect').attr('class', 'wk-chart-brush wk-chart-brush-extent').style('fill', '#ffffff').attr('width', 2000).attr('height', 2000);
      ...
      _chartArea.append('rect').style('visibility', 'hidden').attr('class', 'wk-chart-background').style('pointer-events', 'all').datum({
        name: 'background'
      });
      _chartArea.append('rect').attr('class', 'wk-chart-brush wk-chart-brush-vis wk-chart-brush-extent-marker').style({
        visibility: 'hidden',
        'pointer-events': 'none'
      });
      _container.append('g').attr('class', 'wk-chart-marker-area');
      _container.append('rect').attr('class', 'wk-chart-brush wk-chart-brush-vis wk-chart-brush-line1').style({
        visibility: 'hidden',
        'pointer-events': 'none'
      });
      _container.append('rect').attr('class', 'wk-chart-brush wk-chart-brush-vis wk-chart-brush-line2').style({
        visibility: 'hidden',
        'pointer-events': 'none'
      });
      _container.append('g').attr('class', 'wk-chart-brush wk-chart-brush-vis wk-chart-brush-label1').call(_genBrushLabel).style({
        visibility: 'hidden',
        'pointer-events': 'none'
      });
      return _container.append('g').attr('class', 'wk-chart-brush wk-chart-brush-vis wk-chart-brush-label2').call(_genBrushLabel).style({
        visibility: 'hidden',
        'pointer-events': 'none'
      });
    };
    me.drawChartFrame = function(data, notAnimated) {
      var _frameHeight, _frameWidth, _height, _maxDim, _width, axis, axisRect, bounds, dataLabelRect, dataLabels, i, k, l, label, labelHeight, leftMargin, len, ref, ref1, ref2, s, titleAreaHeight, topMargin;
      ...
      _svg.selectAll('defs>mask>rect.wk-chart-pattern-mask').attr({
        'x': -_maxDim,
        y: -_maxDim,
        height: _maxDim * 2,
        width: _maxDim * 2
      ...
      _spacedContainer.select('.wk-chart-area');
    ...
    getDrawArea = function() {
      ...
      if (drawArea.empty()) {
        drawArea = container.append('g').attr('class', function(d) {
          ...
          mask: "url(#wk-chart-brush-mask-" + (_container.id()) + ")",
          'clip-path': "url(#wk-chart-clip-" + (_container.id()) + ")"
        });
      }
      return drawArea;
    };
...
angular.module('wk.chart').factory('scale', function($log, legend, formatDefaults, wkChartScales, wkChartLocale, maxTicks) {
  ...
  scale = function() {
    ...
    me.formatValue = function(val) {
      if (_outputFormatString && val && (_.isDate(val) || _.isFinite(val))) {
        return _outputFormatFn(val);
      } else {
        return val;
      }
    };
    me.parsedValue = function(val) {
    ...
    me.register = function() {
      me.chart().lifeCycle().on("scaleDomains." + (me.id()), function(data) {
        var domain;
        if (me.parentScale()) {
          return _scale.domain(me.parentScale().scale().domain());
        } else {
...
angular.module('wk.chart').factory('wkArea', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  areaCntr = 0;
  wkArea = function() {
    var _areaStyle, _id, _initialOpacity, _layout, _scaleList, _showMarkers, _showOpacity, _spline, _tooltip, area, drawPath, layoutData, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    _tooltip = void 0;
    _scaleList = {};
...
angular.module('wk.chart').factory('wkAreaStacked', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  wkAreaStacked = function() {
    var _areaStyle, _id, _layout, _scaleList, _showMarkers, _showOpacity, _spline, _tooltip, area, drawPath, layers, layoutData, markers, me, offs, offset, scaleY, setAnimationEnd, setAnimationStart, stack, stackLayout, ttHelper, xData;
    me = function() {};
    _layout = void 0;
...
angular.module('wk.chart').factory('wkAreaStackedVertical', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  wkAreaStackedVertical = function() {
    var _areaStyle, _id, _layout, _scaleList, _showMarkers, _showOpacity, _spline, _tooltip, area, drawPath, layers, layoutData, markers, me, offs, offset, setAnimationEnd, setAnimationStart, stack, stackLayout, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    stack = d3.layout.stack();
    offset = 'zero';
...
angular.module('wk.chart').factory('wkAreaVertical', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  wkAreaVertical = function() {
    var _areaStyle, _circles, _id, _initialOpacity, _layout, _scaleList, _showMarkers, _showOpacity, _spline, _tooltip, _ttHighlight, area, drawPath, layoutData, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    _tooltip = void 0;
    _ttHighlight = void 0;
    ...
    drawPath = function(doAnimate, data, options, x, y, color) {
      ...
      path = layers.select('.wk-chart-area-path').attr('transform', "translate(0," + offset + ")").style('pointer-events', 'none');
      path.each(setStyle);
      path = doAnimate ? path.transition().duration(options.duration) : path;
      path.attr('d', function(d) {
        return area(d.values);
...
angular.module('wk.chart').factory('wkBar', function($log, utils, barConfig, dataLabelFactory, dataManagerFactory, tooltipHelperFactory) {
  ...
  sBarCntr = 0;
  wkBar = function() {
    var _barStyle, _id, _layout, _scaleList, _selected, _tooltip, barOuterPaddingOld, barPaddingOld, bars, config, dataLabels, drawPath, me, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _id = "bars" + (sBarCntr++);
    _layout = void 0;
    bars = null;
...
angular.module('wk.chart').factory('wkBarClustered', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  wkBarClustered = function() {
    var _barStyle, _id, _layout, _scaleList, _selected, _tooltip, barHeight, barOuterPaddingOld, barPaddingOld, clusterY, config, drawPath, initial, layers, me, setAnimationEnd, setAnimationStart, stack, ttHelper, xData;
    me = function() {};
    _layout = void 0;
...
angular.module('wk.chart').factory('wkBarStacked', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  wkBarStacked = function() {
    var _barStyle, _id, _layout, _scaleList, _selected, _tooltip, config, drawPath, layers, me, setAnimationEnd, setAnimationStart, stack, ttHelper, xData;
    me = function() {};
    _layout = void 0;
...
angular.module('wk.chart').factory('wkBoxPlot', function($log, utils, barConfig, dataManagerFactory, markerFactory, tooltipHelperFactory) {
  ...
  sBarCntr = 0;
  wkBoxPlot = function() {
    var _boxStyle, _id, _layout, _scaleList, _selected, _showMarkers, _tooltip, config, drawPath, layoutData, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    ...
    drawPath = function(doAnimate, data, options, x, y, color) {
      ...
      setStyle = function(s, key, noStroke) {
      ...
      boxEnter.append('rect').attr('class', 'wk-chart-box-uq wk-chart-selectable').classed('wk-chart-hidden', true).style('opacity', 0).call(_tooltip.tooltip);
      boxEnter.append('rect').attr('class', 'wk-chart-box-lq  wk-chart-selectable').classed('wk-chart-hidden', true).style('opacity', 0).call(_tooltip.tooltip);
      ...
      uq = box.select('.wk-chart-box-uq').call(setStyle, 'uq', true);
      ...
      lq = box.select('.wk-chart-box-lq').call(setStyle, 'lq', true);
...
angular.module('wk.chart').factory('wkColumn', function($log, utils, barConfig, dataManagerFactory, dataLabelFactory, tooltipHelperFactory) {
  ...
  sBarCntr = 0;
  wkColumn = function() {
    var _columnStyle, _id, _layout, _scaleList, _selected, _tooltip, columns, config, dataLabels, drawPath, me, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    _id = "column" + (sBarCntr++);
    columns = null;
...
angular.module('wk.chart').factory('wkColumnClustered', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  wkColumnClustered = function() {
    var _columnStyle, _id, _layout, _scaleList, _selected, _tooltip, barOuterPaddingOld, barPaddingOld, barWidth, clusterX, config, drawPath, initial, layers, me, setAnimationEnd, setAnimationStart, stack, ttHelper, xData;
    me = function() {};
    _layout = void 0;
...
angular.module('wk.chart').factory('wkColumnStacked', function($log, utils, barConfig, dataManagerFactory, markerFactory, tooltipHelperFactory) {
  ...
  wkColumnStacked = function() {
    var _columnStyle, _id, _layout, _scaleList, _selected, _tooltip, config, drawPath, layers, me, setAnimationEnd, setAnimationStart, stack, ttHelper, xData;
    me = function() {};
    _layout = void 0;
...
angular.module('wk.chart').factory('wkLine', function($log, utils, barConfig, dataLabelFactory, markerFactory, dataManagerFactory, tooltipHelperFactory, timing) {
  ...
  lineCntr = 0;
  wkLine = function() {
    var _id, _layout, _lineStyle, _scaleList, _showMarkers, _spline, _tooltip, drawPath, layoutData, line, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    _tooltip = void 0;
    _showMarkers = false;
...
angular.module('wk.chart').factory('wkLineVertical', function($log, utils, tooltipHelperFactory, dataManagerFactory, markerFactory) {
  ...
  wkLineVertical = function() {
    var _id, _layout, _lineStyle, _scaleList, _showMarkers, _spline, _tooltip, brushStartIdx, drawPath, layoutData, line, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    brushStartIdx = 0;
    _tooltip = void 0;
...
angular.module('wk.chart').factory('wkRangeArea', function($log, utils, dataManagerFactory, markerFactory, tooltipHelperFactory) {
  ...
  wkRangeArea = function() {
    var _areaStyle, _id, _initialOpacity, _layout, _scaleList, _showMarkers, _spline, _tooltip, area, drawPath, layoutData, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    _tooltip = void 0;
    _scaleList = {};
...
angular.module('wk.chart').factory('wkRangeBar', function($log, utils, barConfig, dataManagerFactory, markerFactory, tooltipHelperFactory) {
  ...
  sBarCntr = 0;
  wkRangeBar = function() {
    var _barStyle, _id, _layout, _scaleList, _selected, _showMarkers, _tooltip, config, drawPath, layoutData, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
    ...
    drawPath = function(doAnimate, data, options, x, y, color) {
      ...
      range.enter().append('rect').attr('class', 'wk-chart-rect wk-chart-selectable').style('opacity', 0).call(_tooltip.tooltip).call(_selected).attr('transform', function(d) {
...
angular.module('wk.chart').factory('wkRangeColumn', function($log, utils, barConfig, dataManagerFactory, markerFactory, tooltipHelperFactory) {
  ...
  wkRangeColumn = function() {
    var _columnStyle, _id, _layout, _scaleList, _selected, _showMarkers, _tooltip, config, drawPath, layoutData, markers, me, offset, setAnimationEnd, setAnimationStart, ttHelper, xData;
    me = function() {};
    _layout = void 0;
