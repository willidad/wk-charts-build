...
angular.module('wk.chart').directive('barClustered', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  return {
    ...
    link: function(scope, element, attrs, controller) {
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        var barOuterPadding, barPadding, bars, clusterHeight, layerKeys, offset, stackLayout;
...
angular.module('wk.chart').directive('columnClustered', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  return {
    ...
    link: function(scope, element, attrs, controller) {
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        var barOuterPadding, barPadding, bars, clusterWidth, layerKeys, offset, stackLayout;
        layerKeys = data.filter(function(d) {
          return !d.added && !d.deleted;
        }).map(function(d) {
          return d.layerKey;
        ...
        offset = function(d) {
          if (x.reverse()) {
            ...
            if (d.deleted) {
              return clusterWidth + barPadding / 2;
            }
            if (d.added && d.atBorder) {
              return barPadding / 2;
            }
