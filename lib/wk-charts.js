...
angular.module('wk.chart').factory('chart', function($log, scaleList, container, behavior, d3Animation) {
  ...
  chart = function() {
    ...
    me.prepareScaleHierarchy = function() {
      ...
      for (id in _ref) {
        s = _ref[id];
        if (!_ownedScales.hasScale(s) && _ownedScales.hasKind(s.kind())) {
          if (s.orientation() === _ownedScales.getKind(s.kind()).orientation()) {
            _results.push(s.parentScale(_ownedScales.getKind(s.kind())));
          } else {
