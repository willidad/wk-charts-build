...
angular.module('wk.chart').directive('bars', function($log, utils, barConfig, dataLabelFactory, dataManagerFactory, tooltipHelperFactory) {
  ...
  return {
    ...
    link: function(scope, element, attrs, controller) {
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        var barHeight, barOuterPadding, barPadding, enter, offset, rect, _colorByKey;
...
angular.module('wk.chart').directive('boxPlot', function($log, utils, barConfig, dataManagerFactory, markerFactory, tooltipHelperFactory) {
  ...
  return {
    ...
    link: function(scope, element, attrs, controller) {
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        var barPadding, barWidth, box, boxEnter, i, j, lWhisker, lwConn, median, p, prop, q, r, ranges, showMax, showMin, uWhisker, uwConn, _i, _len;
        ...
        boxEnter.append('rect').attr('class', 'wk-chart-box-rect').style('opacity', 0).style('fill', 'white').style('stroke', 'black').call(_tooltip.tooltip).call(_selected);
        boxEnter.append('line').attr('class', 'wk-chart-box-med').style({
          stroke: 'black',
          opacity: 0
        });
        boxEnter.append('line').attr('class', 'wk-chart-box-lw').style({
          stroke: 'black',
          opacity: 0
        });
        boxEnter.append('line').attr('class', 'wk-chart-box-lw-conn').style({
          stroke: 'black',
          opacity: 0,
          'stroke-dasharray': '2.2'
        });
        boxEnter.append('line').attr('class', 'wk-chart-box-uw').style({
          stroke: 'black',
          opacity: 0
        });
        boxEnter.append('line').attr('class', 'wk-chart-box-uw-conn').style({
          stroke: 'black',
          opacity: 0,
          'stroke-dasharray': '2.2'
        });
        ...
        (doAnimate ? q.transition().duration(options.duration) : q).attr('y', function(d) {
          return y.scale()(d.uq);
        }).attr('height', function(d) {
          return Math.abs(y.scale()(d.uq) - y.scale()(d.lq));
        }).attr('width', function(d) {
          if (d.added || d.deleted) {
            return 0;
          } else {
        ...
        (doAnimate ? median.transition().duration(options.duration) : median).attr('x1', 0).attr('x2', function(d) {
          if (d.added || d.deleted) {
            return 0;
          } else {
            return barWidth;
