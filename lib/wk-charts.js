...
angular.module('wk.chart').factory('behaviorBrush', function($log, $window, selectionSharing, d3Animation) {
  var behaviorBrush;
  behaviorBrush = function() {
    var _active, _areaBox, _areaNode, _backgroundBox, _boundsDomain, _boundsIdx, _boundsValues, _brushEvents, _brushGroup, _brushX, _brushXY, _brushY, _chart, _chartArea, _container, _data, _evTargetData, _extent, _lastBottomVal, _lastLeftVal, _lastRightVal, _lastTopVal, _overlay, _selectables, _startPos, _tooltip, _x, _y, bottom, brushEnd, brushMove, brushStart, clearBrushElements, clearSelection, getD3Selection, getSelectedObjects, hideBrushElements, left, me, positionBrushElements, resizeExtent, right, setSelection, showBrushElements, startBottom, startLeft, startRight, startTop, top;
    me = function() {};
    _active = false;
    _overlay = void 0;
    _extent = void 0;
    ...
    _boundsDomain = [];
    _lastLeftVal = void 0;
    _lastRightVal = void 0;
    _lastBottomVal = void 0;
    _lastTopVal = void 0;
    _brushEvents = d3.dispatch('brushStart', 'brush', 'brushEnd');
    left = top = right = bottom = startTop = startLeft = startRight = startBottom = void 0;
    getD3Selection = function(s, animate) {
      if (animate) {
        return _overlay.selectAll(s).transition().duration(d3Animation.duration);
      } else {
        return _overlay.selectAll(s);
      }
    };
    positionBrushElements = function(left, right, top, bottom, animate) {
      var height, width;
      ...
      if (_brushX) {
        ...
        (animate ? _extent.transition().duration(d3Animation.duration) : _extent).attr('width', width).attr('height', _areaBox.height).attr('x', left).attr('y', 0);
      }
      if (_brushY) {
        ...
        return (animate ? _extent.transition().duration(d3Animation.duration) : _extent).attr('width', _areaBox.width).attr('height', height).attr('x', 0).attr('y', top);
      }
    };
    ...
    showBrushElements = function() {
      d3.select(_areaNode).selectAll(".wk-chart-resize").style("display", null);
      return _extent.style('display', null);
    };
    clearBrushElements = function() {
      d3.select(_areaNode).selectAll(".wk-chart-resize.wk.chart-n>rect, .wk-chart-resize.wk.chart-s>rect").attr('x', 0).attr('width', 0);
      return d3.select(_areaNode).selectAll(".wk-chart-resize.wk.chart-e>rect, .wk-chart-resize.wk.chart-e>rect").attr('y', 0).attr('height', 0);
    };
    ...
    me.data = function(val) {
      if (arguments.length === 0) {
        ...
        _lastLeftVal = _lastRightVal = _lastTopVal = _lastBottomVal = void 0;
        _data = val;
        if (_brushX && _x.isOrdinal()) {
          _boundsValues = _.intersection(_x.value(_data), _boundsValues);
        }
        if (_brushY && _y.isOrdinal()) {
          _boundsValues = _.intersection(_y.value(_data), _boundsValues);
        }
        if (_boundsValues && _boundsValues.length > 0) {
          me.setExtent([_boundsValues[0], _boundsValues[_boundsValues.length - 1]], true);
        } else {
          me.clearBrush();
        }
        return me;
    ...
    me.setExtent = function(val, noParse) {
      var end, endPos, start, startPos, v;
      if (_.isArray(val) && val.length === 2) {
        ...
        _areaBox = _areaNode.getBBox();
        if (_brushX) {
          left = _x.scale()(noParse ? start : _x.parsedValue(start)) + (_x.isOrdinal() ? _x.scale().rangeBand() / 2 : 0);
          right = _x.scale()(noParse ? end : _x.parsedValue(end)) + (_x.isOrdinal() ? _x.scale().rangeBand() / 2 : 0);
          if (_x.reverse()) {
            v = left;
            left = right;
            right = v;
          ...
          _.delay(function() {
            ...
            return showBrushElements();
          });
        } else if (_brushY) {
          top = _y.scale()(noParse ? end : _y.parsedValue(end)) + (_y.isOrdinal() ? _y.scale().rangeBand() / 2 : 0);
          bottom = _y.scale()(noParse ? start : _y.parsedValue(start)) + (_y.isOrdinal() ? _y.scale().rangeBand() / 2 : 0);
          if (_y.reverse()) {
            v = top;
            top = bottom;
            bottom = v;
