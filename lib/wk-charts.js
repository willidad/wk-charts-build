        ...
        while (i >= 0) {
          ...
          if (cur.iNew !== void 0) {
            ...
            ret.unshift({
              ...
              targetKey: (cur.atBorder && !_isOrdinal ? cur.key : lastKey),
              key: cur.key,
              data: cur.deleted ? _dataOld[cur.iOld] : _dataNew[lastNew],
              targetData: _dataOld[cur.iOld],
              oldData: _dataOld[cur.iOld]
            });
          }
        ...
        return {
          ...
          values: series.map(function(d) {
            return {
              ...
              lowBorder: d.lowBorder,
              highBorder: d.highBorder,
              value: d.deleted ? _valueScale.layerValue(d.targetData, layer.key) : _valueScale.layerValue(d.data, layer.key),
...
angular.module('wk.chart').directive('barClustered', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  return {
    ...
    require: '^layout',
    link: function(scope, element, attrs, controller) {
      var barHeight, barOuterPaddingOld, barPaddingOld, clusterY, config, drawBrush, drawPath, host, initial, layers, setAnimationEnd, setAnimationStart, stack, ttHelper, xData, _id, _scaleList, _selected, _tooltip;
      host = controller.me;
...
angular.module('wk.chart').directive('columnClustered', function($log, utils, barConfig, dataManagerFactory, tooltipHelperFactory) {
  ...
  return {
    ...
    require: '^layout',
    link: function(scope, element, attrs, controller) {
      var barOuterPaddingOld, barPaddingOld, barWidth, clusterX, config, drawBrush, drawPath, host, initial, layers, setAnimationEnd, setAnimationStart, stack, ttHelper, xData, _id, _scaleList, _selected, _tooltip;
      host = controller.me;
    ...
    link: function(scope, element, attrs, controller) {
      ...
      drawPath = function(doAnimate, data, options, x, y, color) {
        ...
        enter.append('path').attr('class', 'wk-chart-line').attr('d', function(d) {
          return line(d.values);
        }).style('opacity', 0).style('pointer-events', 'none').style(_lineStyle).style('stroke', function(d) {
          return color.scale()(d.layerKey);
        });
        if (doAnimate) {
...
angular.module('wk.chart').factory('container', function($log, $window, wkChartMargins, scaleList, axisConfig, d3Animation, behavior) {
  ...
  container = function() {
    ...
    me.drawChartFrame = function(data, notAnimated) {
      var axis, axisRect, bounds, dataLabelHeight, dataLabelRect, dataLabelWidth, dataLabels, k, l, label, labelHeight, leftMargin, s, titleAreaHeight, topMargin, _frameHeight, _frameWidth, _height, _i, _len, _ref, _ref1, _ref2, _width;
