        ...
        for (k in _ref1) {
          ...
          if (k === 'x' || k === 'rangeX') {
            ...
            if (l.showDataLabels() === 'y') {
              s.range(s.reverse() ? [0, _innerHeight - dataLabelHeight] : [_innerWidth - dataLabelHeight, 0]);
            } else {
...
angular.module('wk.chart').service('tooltipUtils', function($log) {
  this.createTooltipMarkers = function(selection, idx) {
    ...
    selection.append('circle').attr('class', 'wk-charts-tt-marker').attr('r', 9).style('fill', function(d) {
      if (hasIdx) {
        return d[idx].color;
      } else {
        return d.color;
    ...
    return selection.append('circle').attr('r', 4).style('fill', function(d) {
      if (hasIdx) {
        return d[idx].color;
      } else {
        return d.color;
