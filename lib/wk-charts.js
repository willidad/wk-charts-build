...
angular.module('wk.chart').factory('legend', function($log, $compile, $rootScope, $templateCache, wkChartTemplates) {
  ...
  legend = function() {
    var destroyme, me, _containerDiv, _data, _id, _layout, _legendDiv, _legendScope, _legendStyle, _options, _parsedTemplate, _position, _scale, _show, _showValues, _template, _templatePath, _title;
  ...
  ttHelpers = function() {
    ...
    me.enter = function(data) {
      ...
      for (_i = 0, _len = layerKeys.length; _i < _len; _i++) {
        key = layerKeys[_i];
        this.layers[key] = {};
        this.layers[key].value = _valueScale.formatValue(data[key]);
        if (_colorScale.property().length > 0) {
          _results.push(this.layers[key].color = _colorScale.map(data));
        } else if (_colorByKey) {
